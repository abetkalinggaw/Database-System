...
$setrekening = query("SELECT * FROM setuprekening");
?>
...
<body>
    ...
    <div class="container">
        <div class="row my-2">
            <div class="col-md">
                <h3 class="text-center fw-bold text-uppercase">Data Setup Rekening</h3>
                <hr>
            </div>
        </div>
        <div class="row my-2">
            <div class="col-md">
                <a href="setupRekening_tambah.php" class="btn btn-primary"><i class="bi bi-person-plus-fill"></i>&nbsp;Tambah
                    Data</a>
            </div>
        </div>
        <div class="row my-3">
            <div class="col-md">
                <table id="data" class="table table-striped table-responsive table-hover text-center"
                    ...
                    <tbody>
                        <?php $kodeSetup= 1; ?>
                        <?php foreach ($setrekening as $row) : ?>
                        <tr>
                            <td>
                                <a href="setupRekening_update.php?kodeSetup=<?= $row['kodeSetup']; ?>"
                                    class="btn btn-warning btn-sm" style="font-weight: 600;"><i
                                        class="bi bi-pencil-square"></i>&nbsp;Update</a>
                                <a href="setupRekening_hapus.php?kodeSetup=<?= $row['kodeSetup']; ?>"
                                    class="btn btn-danger btn-sm" style="font-weight: 600;"
                                    onclick="return confirm('Apakah anda yakin ingin menghapus data <?= $row['kodeSetup']; ?> ?');"><i
                                        class="bi bi-trash-fill"></i>&nbsp;Delete</a>
                            </td>
                            <td><?= $row["kodeSetup"];?></td>
                            <td><?= $row["kodeNasabah"];?></td>
                            <td><?= $row["noRekening"];?></td>
                            <td><?= $row["tanggalPembukuan"];?></td>
