...
$nasabah = query("SELECT * FROM nasabah WHERE kodeNasabah = $kodeNasabah")[0];
...
if (isset($_POST['submit'])) {
    if (update($_POST) > 0) {
        ...
        echo "
            <script>
                alert('Data Nasabah Gagal Diupdate!');
                document.location.href = 'index.php';
            </script>
        ";
    }
                ...
                <form action="" method="post" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="email" class="form-control w-50" id="email" placeholder="Masukkan E-Mail"
                            name="email" required value="<?= $nasabah["email"];?>">
                    </div>
                    <div class="mb-3">
                        <label for="noHP" class="form-label">Nomor HP</label>
                        <input type="text" class="form-control w-50" id="noHP" placeholder="Masukkan Nomor HP"
                            name="noHP" required value="<?= $nasabah["noHP"];?>">
                    </div>
                    <div class="mb-3">
                        <label for="alamat" class="form-label">Alamat</label>
                        <input class="form-control w-50" id="alamat" rows="5" name="alamat"
                            placeholder="Masukkan Alamat" required value="<?= $nasabah["alamat"];?>">
                    </div>
                    <div class="mb-3">
                        <label for="namaIbuKandung" class="form-label">Nama Ibu Kandung</label>
                        <input class="form-control w-50" id="namaIbuKandung" name="namaIbuKandung"
                            placeholder="Masukkan Nama Ibu Kandung" required
                            value="<?= $nasabah["namaIbuKandung"];?>">
                    </div>
                    <hr>
